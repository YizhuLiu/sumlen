#i Copyright (c) 2017-present, Facebook, Inc.
# All rights reserved.
#
...
class SequenceGenerator(object):
    ...
    def generate_batched_itr(self, data_itr, beam_size=None, maxlen_a=0.0, maxlen_b=None,
        ...
        for sample in data_itr:
            ...
            with utils.maybe_no_grad():
                #print(input['src_tokens'])
                #print(input['input_token_lengths'])
                hypos = self.generate(input['src_tokens'], input['input_token_lengths'], beam_size=beam_size,
    ...
    def generate(self, src_tokens, input_token_lenghts, beam_size=None, maxlen=None):
        """Generate a batch of translations."""
        with ExitStack() as stack:
            for model in self.models:
                if isinstance(model.decoder, FairseqIncrementalDecoder):
                    stack.enter_context(model.decoder.incremental_inference())
            return self._generate(src_tokens, input_token_lenghts, beam_size, maxlen)

    def _generate(self, src_tokens, input_token_lengths, beam_size=None, maxlen=None):
        ...
        for step in range(maxlen + 1):  # one extra step for EOS marker
            ...
            probs, avg_attn_scores = self._decode(tokens[:, :step+1], input_token_lengths, encoder_outs)
            if step == 0:
    ...
    def _decode(self, tokens, input_token_lengths, encoder_outs):
        # wrap in Variable
        ...
        for model, encoder_out in zip(self.models, encoder_outs):
            with utils.maybe_no_grad():
                decoder_out, attn = model.decoder(tokens, input_token_lengths, encoder_out)
