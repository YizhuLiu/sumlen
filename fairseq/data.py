...
def load_dataset(path, load_splits, src=None, dst=None):
    ...
    #print(dataset.src_dict.pad()) 1
    #print(dataset.src_dict.eos()) 2
    #print(dataset.src_dict.unk()) 3 
    ...
    for split in load_splits:
        for k in itertools.count():
            ...
            targetlens_path = fmt_path('{}.{}.{}',prefix,'len',dst)

            if not IndexedInMemoryDataset.exists(src_path):
                break

            targetlens_file = open(targetlens_path)
            targetlens = [int(l.strip()) for l in targetlens_file.readlines()]
            #print(IndexedInMemoryDataset(dst_path))
            #print(np.array(targetlens))
            #print(type(IndexedInMemoryDataset(dst_path)))
            #print(type(targetlens))
            dataset.splits[prefix] = LanguagePairDataset(
                ...
                target_lens=np.array(targetlens),
            )
...
class LanguagePairDataset(object):
    ...
    def __init__(self, src, dst, pad_idx, eos_idx, target_lens):
        self.src = src
        self.dst = dst
    ...
    def collate(samples, pad_idx, eos_idx):
        ...
        def merge(key, left_pad, move_eos_to_beginning=False):
            return LanguagePairDataset.collate_tokens(
                [s[key] for s in samples], pad_idx, eos_idx, left_pad, move_eos_to_beginning, [s['target_lens'] for s in samples])
        ...
        return {
            ...
            'net_input': {
                ...
                'input_token_lengths': torch.FloatTensor(np.array([s['target_lens'] for s in samples]))
            },
            'target': merge('target', left_pad=LanguagePairDataset.LEFT_PAD_TARGET),
        }

    @staticmethod
    def collate_tokens(values, pad_idx, eos_idx, left_pad, move_eos_to_beginning, target_lens):
