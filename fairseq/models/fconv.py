...
class FConvDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, dictionary, embed_dim=512, out_embed_dim=256,
        ...
        num_embeddings = len(dictionary)#number of word in target_vocab
        padding_idx = dictionary.pad()
        #print(embed_dim)
        self.embed_tokens = Embedding(num_embeddings, embed_dim, padding_idx)
        self.embed_positions = PositionalEmbedding(max_positions, embed_dim, padding_idx,
    ...
    def forward(self, input_tokens, input_token_lengths, encoder_out):
        ...
        #print(positions)#num(doc)x(maxlen of doc)xembed_dim
        ...
        # project to size of convolution
        x = self.fc1(x)
        length = input_token_lengths/100
        #print(type(x))
        # B x T x C -> T x B x C
        x = self._transpose_unless_incremental_eval(x)
        #print(x)
        #length = length.view(length.size(0),1).expand(len(length),x.size(2))
        #print(length)
        #print(length.size(0))
        if length.size(0) != x.size(1):
           #test-decoder
           length = length.view(length.size(0),1).expand(len(length),int(x.size(0)/len(length)))
           length = length.contiguous().view(x.size(0),1)
           length = length.expand(len(length),x.size(2))
           length = torch.unsqueeze(length,0)
           #length = length.cuda().permute(1,0,2)
           length = length.permute(1,0,2)
           x = x*length
        else:
           length = length.view(length.size(0),1).expand(len(length),x.size(2))
           x = x*length
        ...
        #print(type(input_token_lengths)) 
        # temporal convolutions
        avg_attn_scores = None
        num_attn_layers = len(self.attention)
        for proj, conv, attention in zip(self.projections, self.convolutions, self.attention):
            residual = x if proj is None else proj(x)
            #print(residual)#72x2x512  maxlenxdocNumxhiddenlayer
...
def get_archs():
    return [
        'fconv', 'fconv_iwslt_de_en', 'fconv_wmt_en_ro', 'fconv_wmt_en_de', 'fconv_wmt_en_fr', 'fconv_cnndm_en_de'
    ]
...
def parse_arch(args):
    ...
    if args.arch == 'fconv_iwslt_de_en':
        ...
        args.decoder_layers = convs
        args.decoder_out_embed_dim = 512
    elif args.arch == 'fconv_cnndm_en_de':
        args.encoder_embed_dim = 256 
        args.encoder_layers = '[(512, 3)] * 8'
        args.decoder_embed_dim = 256
        args.decoder_layers = '[(512, 3)] * 8'
        args.decoder_out_embed_dim = 256
    else:
    ...
    args.encoder_layers = getattr(args, 'encoder_layers', '[(512, 3)] * 20')#20
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 512)
    args.decoder_layers = getattr(args, 'decoder_layers', '[(512, 3)] * 20')#20
